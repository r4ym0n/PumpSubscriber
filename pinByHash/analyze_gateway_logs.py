...
def human_stats(values: List[int]) -> Dict[str, float]:
    if not values:
        return {"count": 0, "avg_ms": float("nan"), "p95_ms": float("nan"), "min_ms": float("nan"), "max_ms": float("nan")}
    return {
        "count": len(values),
...
def analyze(path: str) -> dict:
    ...
    for line in read_lines(path):
        ...
        if not cid or not url or not isinstance(elapsed, int):
            continue
        # derive gateway label from hostname
        gateway = url.split("//", 1)[-1].split("/", 1)[0]
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: python analyze_gateway_logs.py /path/to/pump.log", file=sys.stderr)
        return 2
    path = sys.argv[1]
    result = analyze(path)
